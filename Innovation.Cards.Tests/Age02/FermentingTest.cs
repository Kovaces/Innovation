...
namespace Innovation.Cards.Tests
{
    ...
    public class FermentingTest
    {
        ...
        //ActionType.Required, Symbol.Leaf, "Draw a [2] for every two [LEAF] icons on your board."

        [TestMethod]
        public void Card_FermentingAction1_NoChange()
        {
            var result = new Fermenting().Actions.ToList()[0].ActionHandler(new CardActionParameters { TargetPlayer = testGame.Players[0], Game = testGame, ActivePlayer = testGame.Players[0], PlayerSymbolCounts = new Dictionary<IPlayer, Dictionary<Symbol, int>>() });
        ...
        [TestMethod]
        public void Card_FermentingAction1_ThreeLeafs()
        {
            testGame.Players[0].Tableau.Stacks[Color.Purple].AddCardToTop(
